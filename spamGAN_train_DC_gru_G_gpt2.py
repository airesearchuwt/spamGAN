...
#         config = importlib.import_module('spamGAN_config_minimal')
        config = importlib.import_module('spamGAN_config_smallunsup_opspam_cpu')
    else:
...
#         vocab_size = vocab.size
        vocab_size = config.gen_hparams["gpt2_decoder"]["vocab_size"] # Vocabulary size of GPT2 
...
#                 tf.concat([tf.ones_like(r_preds), tf.zeros_like(f_preds)], axis=0)))
            disc_acc = tf.reduce_mean(tf.contrib.metrics.accuracy(
                predictions=tf.concat([r_preds, f_preds], axis=0), 
                labels=tf.concat([tf.ones_like(r_preds), tf.zeros_like(f_preds)], axis=0)))
...
#                 with open('../../perplexities.txt', 'a') as f:
            ...
            if config.clas_test is not None and config.clas_test:
                checkpoint.restore(sess, config.clas_test_ckpt)
                outs = clas_run_epoch(sess, 'test', sum_writer, 0)
                logger.info(outs)
                with open(config.clas_pred_output, 'w') as f:
