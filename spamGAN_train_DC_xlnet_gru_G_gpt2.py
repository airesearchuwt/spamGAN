...
class GPT2Generator(tf.keras.Model):
    """Generator wrapper for checkpointing"""
    def __init__(self, gen_hparams):
        super(GPT2Generator, self).__init__()
        self.decoder = tx.modules.GPT2Decoder(hparams=gen_hparams)
...
#         gen_model = Generator(vocab_size, config.gen_hparams["rnn_decoder"], generator_dropout)
        gen_model = GPT2Generator(config.gen_hparams["gpt2_decoder"])
...
#                 sequence_length=x_lengths)
            ...
            outputs_mle = g_decoder(
                ...
                inputs=x,
                mode=generator_dropout,
                spamGAN_mle_context=tiled_context
                )           
...
#             gen_inputs_emb_context = tf.concat([gen_inputs_emb, tiled_random_vector], axis = -1)
            ...
            gen_outputs = g_decoder(
                ...
                inputs=gen_inputs,
                mode=generator_dropout,
                spamGAN_mle_context=tiled_random_vector
                )
