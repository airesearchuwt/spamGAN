...
class TokenizerBase(ModuleBase):
    ...
    def __init__(self, hparams):
        ...
        self.added_tokens_encoder_tensor = tf.lookup.MutableHashTable(
    ...
    def map_token_to_id(self, tokens):
        ...
        """
        for key in self.added_tokens_encoder.keys():
            self.added_tokens_encoder_tensor.insert(
                keys=tf.convert_to_tensor(key), 
                values=tf.convert_to_tensor(added_tokens_encoder.get(key)
                                            )
                )
        
        ids = tf.constant([-1])
        ids = tf.map_fn(
            fn=lambda token: tf.concat(
                [ids, self.added_tokens_encoder_tensor.lookup(tf.convert_to_tensor(token))]),
            elems=tokens,
            dtype=tf.int32
            )
