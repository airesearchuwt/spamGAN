...
from custom_bert_encoder import BERTEncoder
from custom_bert_tokenizer import BERTTokenizer
...
class RNNClassifier(tf.keras.Model):
    def __init__(self, class_config, dropout):
        ...
        self.encoder = tx.modules.UnidirectionalRNNEncoder(
            hparams = class_config['rnn_encoder'], cell_dropout_mode=dropout)
...
class BERTEmbedder(tf.keras.Model):
    def __init__(self, bert_emb_config):
        super(BERTEmbedder, self).__init__()
        self.embedder = BERTEncoder(hparams=bert_emb_config["bert_embedder"])
        self.tokenizer = BERTTokenizer(hparams=bert_emb_config["bert_tokenizer"])
        
    def map_token_list_to_text_list(self, token_list):
        print("token_list: {}".format(token_list))
        text_list = tf.map_fn(
            fn=lambda tokens: tf.strings.strip(
                input=tf.strings.regex_replace(
                    input=tf.strings.reduce_join(inputs=tokens, axis=0, separator=" "), 
                    pattern=" ##", 
                    rewrite=""
                    )
                ),
...
#         config = importlib.import_module('spamGAN_config_minimal')
        config = importlib.import_module('spamGAN_config_minimal_opspam')
    else:
    ...
    with g.as_default():
        ...
        bert_emb_model = BERTEmbedder(config.bert_emb_hparams)
        bert_embedder = bert_emb_model.embedder
        bert_tokenizer = bert_emb_model.tokenizer
...
#             disc_embedder = disc_embedder_model.embedder
            disc_emb_model = BERTEmbedder(config.bert_emb_hparams)
            disc_embedder = disc_emb_model.embedder
        else:
            disc_embedder = bert_embedder
...
#             clas_embedder = clas_emb_model.embedder
            clas_emb_model = BERTEmbedder(config.bert_emb_hparams)
            clas_embedder = clas_emb_model.embedder
        else:
            clas_embedder = bert_embedder
...
#             fake_inp = disc_embedder(fake_seq, mode=discriminator_dropout)
            real_seq_input_ids, real_seq_segment_ids, _ = bert_tokenizer.encode_text(
                bert_emb_model.map_token_list_to_text_list(
                    vocab.map_ids_to_tokens(real_seq)
                    )
                )
            fake_seq_input_ids, fake_seq_segment_ids, _ = bert_tokenizer.encode_text(
                bert_emb_model.map_token_list_to_text_list(
                    vocab.map_ids_to_tokens(fake_seq)
                    )
                )
            real_inp, _ = disc_embedder(
                real_seq_input_ids, 
                segment_ids=real_seq_segment_ids, 
                mode=discriminator_dropout
                )
            fake_inp, _ = disc_embedder(
                fake_seq_input_ids, 
                segment_ids=fake_seq_segment_ids, 
                mode=discriminator_dropout
                )
...
#                 d_variables = tx.utils.collect_trainable_variables([discriminator])
            if config.let_discriminator_train_embedder:
                if config.is_bert_trainable == True:
                    d_variables = tx.utils.collect_trainable_variables([discriminator, disc_embedder])
                else:
                    d_variables = tx.utils.collect_trainable_variables([discriminator])
            else:
...
#             fake_label_inp_emb = clas_embedder(fake_seq, mode=classifier_dropout)
            real_label_input_ids, real_label_segment_ids, _ = bert_tokenizer.encode_text(
                bert_emb_model.map_token_list_to_text_list(
                    vocab.map_ids_to_tokens(real_label_inp)
                    )
                )
            fake_label_input_ids, fake_label_segment_ids, _ = bert_tokenizer.encode_text(
                bert_emb_model.map_token_list_to_text_list(
                    vocab.map_ids_to_tokens(fake_seq)
                    )
                )
            real_label_inp_emb, _ = clas_embedder(
                real_label_input_ids, 
                segment_ids=real_label_segment_ids, 
                mode=classifier_dropout
                )
            fake_label_inp_emb, _ = clas_embedder(
                fake_label_input_ids, 
                segment_ids=fake_label_segment_ids, 
                mode=classifier_dropout
                )
...
#                 c_variables = tx.utils.collect_trainable_variables([classifier])
            if config.clas_has_own_embedder:
                if config.is_bert_trainable is True:
                    c_variables = tx.utils.collect_trainable_variables([classifier, clas_embedder])
                else:
                    c_variables = tx.utils.collect_trainable_variables([classifier])
            else:
