...
def beam_search(symbols_to_logits_fn,
    ...
    # Collect logits while beam searching
    if beam_size == 1:
        logits_list = tf.zeros([batch_size, beam_size, vocab_size]) # Degrade to infer_greedy
    else:
        logits_list = tf.zeros([batch_size, 1, beam_size, vocab_size])
    ...
    def grow_topk(i, alive_seq, alive_log_probs, states, logits_list):
        ...
        # Collect best beam logits
        if beam_size == 1: # Degrade to infer_greedy
            logits_list = tf.concat(
                [logits_list, tf.reshape(logits, [batch_size, beam_size, -1])],
                axis=1
            )
        else:
            logits_list = tf.concat(
                [logits_list, tf.reshape(logits, [batch_size, 1, beam_size, -1])],
                axis=1
            )
        ...
        return topk_seq, topk_log_probs, topk_scores, topk_finished, states, logits_list

    def inner_loop(i, alive_seq, alive_log_probs, finished_seq,
                   finished_scores, finished_flags, states, logits_list):
        ...
        topk_seq, topk_log_probs, topk_scores, topk_finished, states, logits_list =\
            grow_topk(i, alive_seq, alive_log_probs, states, logits_list)
    ...
    (_, alive_seq, alive_log_probs, finished_seq, finished_scores,
     finished_flags, _, logits_list) = tf.while_loop(
        _is_finished,
        inner_loop, [
...
#         back_prop=False)
    ...
    # Remove the first meaningless logits
    if beam_size == 1:
        logits_list = logits_list[:, 1:, :]
    else:
        logits_list = logits_list[:, 1:, :, :]
    
    return finished_seq, finished_scores, logits_list
