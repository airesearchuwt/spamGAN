...
class XLNetEmbedder(tf.keras.Model):
    def __init__(self, xlnet_emb_config):
        super(XLNetEmbedder, self).__init__()
        self.embedder = tx.modules.XLNetEncoder(hparams=xlnet_emb_config)
...
#         config = importlib.import_module('spamGAN_config_minimal')
        config = importlib.import_module('spamGAN_config_smallunsup_gpu')
    else:
    ...
    with g.as_default():
        ...
        xlnet_emb_model = XLNetEmbedder(config.xlnet_emb_hparams)
        xlnet_embedder = xlnet_emb_model.embedder
...
#             disc_embedder = disc_embedder_model.embedder
            disc_embedder_model = XLNetEmbedder(config.xlnet_emb_hparams)
            disc_embedder = disc_embedder_model.embedder
        else:
#             disc_embedder = embedder
            disc_embedder = xlnet_embedder
...
#             clas_embedder = clas_emb_model.embedder
            clas_emb_model = XLNetEmbedder(config.xlnet_emb_hparams)
            clas_embedder = clas_emb_model.embedder
        else:
#             clas_embedder = embedder
            clas_embedder = xlnet_embedder
...
#             fake_inp = disc_embedder(fake_seq, mode=discriminator_dropout)
            real_inp, _ = disc_embedder(real_seq, mode=discriminator_dropout)
            fake_inp, _ = disc_embedder(fake_seq, mode=discriminator_dropout)
...
#             fake_label_inp_emb = clas_embedder(fake_seq, mode=classifier_dropout)
            real_label_inp_emb, _ = clas_embedder(real_label_inp, mode=classifier_dropout)
            fake_label_inp_emb, _ = clas_embedder(fake_seq, mode=classifier_dropout)
...
#                 c_variables = tx.utils.collect_trainable_variables([classifier, clas_embedder])
    ...
    with sess:
        ...
        with sum_writer:
            ...
            # Test dataset after whole experiment
            logger.info("\nTest after the experiment: ")
            test_outputs = clas_run_epoch(sess, "test", sum_writer, 0)
