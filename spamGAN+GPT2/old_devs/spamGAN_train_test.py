...
#         config = importlib.import_module('spamGAN_config_minimal_backup')
        config = importlib.import_module('spamGAN_config_smallunsup_test')
    else:
...
#             context = tf.concat([context, true_classes], axis=1)
...
#                 max_decoding_length = max_length)
            ...
            gen_inputs_emb = embedder(gen_inputs, mode=generator_dropout)
#             tiled_random_vector = tf.reshape(
#                 tf.tile(random_vector, [1, tf.shape(x)[1]]), [-1, tf.shape(x)[1], context_size + 1])
            tiled_random_vector = tf.reshape(
