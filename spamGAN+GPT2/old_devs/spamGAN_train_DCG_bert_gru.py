...
#         emb_model = Embedder(vocab_size, config.emb_hparams)
        emb_model = BERTEmbedder(config.bert_emb_hparams)
...
#             disc_embedder_model = Embedder(vocab_size, config.disc_emb_hparams)
            disc_embedder_model = BERTEmbedder(config.bert_emb_hparams)
...
#             inp_emb = embedder(inp, mode=generator_dropout)
            inp_emb, _ = embedder(inp, mode=generator_dropout)

            x = inp[:, 0:(tf.shape(inp)[1] -2)]
#             x_emb = embedder(x, mode=generator_dropout)
            x_emb, _ = embedder(x, mode=generator_dropout)
...
#                 max_decoding_length = max_length)
            ...
            gen_inputs_emb, _ = embedder(gen_inputs, mode=generator_dropout)
