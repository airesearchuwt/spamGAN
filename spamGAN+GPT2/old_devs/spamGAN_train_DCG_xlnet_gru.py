...
#             inp_emb = embedder(inp, mode=generator_dropout)
            inp_emb, _ = xlnet_embedder(inp, mode=generator_dropout)
            x = inp[:, 0:(tf.shape(inp)[1] -2)]
#             x_emb = embedder(x, mode=generator_dropout)
            x_emb, _ = xlnet_embedder(x, mode=generator_dropout)
...
#             g_variables = tx.utils.collect_trainable_variables([xlnet_embedder, g_decoder])
...
#                 max_decoding_length = max_length)
            ...
            gen_inputs_emb, _ = xlnet_embedder(gen_inputs, mode=generator_dropout)
            tiled_random_vector = tf.reshape(
                tf.tile(random_vector, [1, tf.shape(x)[1]]), [-1, tf.shape(x)[1], context_size + class_size])
            gen_inputs_emb_context = tf.concat([gen_inputs_emb, tiled_random_vector], axis = -1)
...
#                 d_variables = tx.utils.collect_trainable_variables([discriminator, disc_embedder])
                d_variables = tx.utils.collect_trainable_variables([discriminator, xlnet_embedder])
            else:
...
#                 c_variables = tx.utils.collect_trainable_variables([classifier, clas_embedder])
                c_variables = tx.utils.collect_trainable_variables([classifier, xlnet_embedder])
            else:
...
#             pg_variables = tx.utils.collect_trainable_variables([g_decoder, xlnet_embedder])
