...
class GPT2Embedder(tf.keras.Model):
    def __init__(self, gpt2_emb_config):
        ...
        self.embedder = GPT2Decoder(hparams=gpt2_emb_config)
...
#             disc_embedder_model = Embedder(vocab_size, config.disc_emb_hparams)
            disc_embedder_model = GPT2Embedder(config.gpt2_emb_hparams)
...
#             clas_emb_model = Embedder(vocab_size, config.clas_emb_hparams)
            clas_emb_model = GPT2Embedder(config.gpt2_emb_hparams)
...
#                 max_decoding_length=max_length)
            ...
            if sample_strategy == "infer_sample":
                ...
                gen_lengths = get_gen_lengths(gen_sample_ids)
                print("gen_logits: {}".format(gen_logits))
...
#             fake_inp = disc_embedder(fake_seq, mode=discriminator_dropout)
            r_disc_emb_outputs = disc_embedder(
                decoding_strategy='train_greedy',
                inputs=real_seq,
                max_decoding_length=real_seq_lengths,
                mode=discriminator_dropout,
                )
            real_inp = r_disc_emb_outputs.logits
            f_disc_emb_outputs = disc_embedder(
                decoding_strategy='train_greedy',
                inputs=fake_seq,
                max_decoding_length=gen_lengths,
                mode=discriminator_dropout,
                )
            fake_inp = f_disc_emb_outputs.logits
...
#                 return_cell_output=True)
            ...
            r_disc_q_logit = disc_logits_output_layer(tf.concat([r_disc_outputs_fw, r_disc_outputs_bw], axis=-1))
            f_disc_q_logit = disc_logits_output_layer(tf.concat([f_disc_outputs_fw, f_disc_outputs_bw], axis=-1))
            r_disc_cell_outputs = disc_cell_output_layer(tf.concat([r_disc_cell_outputs_fw, r_disc_cell_outputs_bw], axis=-1))
            f_disc_cell_outputs = disc_cell_output_layer(tf.concat([f_disc_cell_outputs_fw, f_disc_cell_outputs_bw], axis=-1))
...
#             fake_label_inp_emb = clas_embedder(fake_seq, mode=classifier_dropout)
            r_clas_emb_outputs = clas_embedder(
                decoding_strategy='train_greedy',
                inputs=real_label_inp,
                max_decoding_length=label_seq_lengths,
                mode=classifier_dropout,
                )
            real_label_inp_emb = r_clas_emb_outputs.logits
            f_clas_emb_outputs = clas_embedder(
                decoding_strategy='train_greedy',
                inputs=fake_seq,
                max_decoding_length=gen_lengths,
                mode=classifier_dropout,
                )
            fake_label_inp_emb = f_clas_emb_outputs.logits
...
#                 mode=classifier_dropout, return_cell_output=True)
            ...
            # Concatenate outputs of FW and BW
            r_clas_q_logit = clas_logits_output_layer(tf.concat([r_clas_outputs_fw, r_clas_outputs_bw], axis=-1))
            f_clas_q_logit = clas_logits_output_layer(tf.concat([f_clas_outputs_fw, f_clas_outputs_bw], axis=-1))
            r_clas_cell_outputs = clas_cell_output_layer(tf.concat([r_clas_cell_outputs_fw, r_clas_cell_outputs_bw], axis=-1))
            f_clas_cell_outputs = clas_cell_output_layer(tf.concat([f_clas_cell_outputs_fw, f_clas_cell_outputs_bw], axis=-1))
