...
#         config = importlib.import_module('stepGAN_config_minimal')
        config = importlib.import_module('spamGAN_base_config_smallunsup')
    g = tf.Graph()
    with g.as_default():
...
#             gen_lengths = tf.clip_by_value(gen_lengths, 0, tf.shape(fake_inp)[1])
            gen_lengths = tf.clip_by_value(tf.cast(gen_lengths, dtype=tf.int32), 0, tf.shape(fake_inp)[1])
...
#             tf.summary.scalar('pg_loss', pg_loss)
            tf.summary.scalar('pg_loss', tf.reduce_mean(pg_loss))
