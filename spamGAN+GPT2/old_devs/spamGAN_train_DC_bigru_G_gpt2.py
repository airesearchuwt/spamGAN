...
from custom_gpt2_decoder import GPT2Decoder
...
class Generator(tf.keras.Model):
    """Generator wrapper for checkpointing"""
    def __init__(self, gen_hparams):
        super(Generator, self).__init__()
        self.decoder = GPT2Decoder(
            hparams=gen_hparams["gpt2_decoder"])
...
class Discriminator(tf.keras.Model):
    ...
    def __init__(self, disc_config, dropout):
        ...
        self.encoder = tx.modules.BidirectionalRNNEncoder(
            hparams = disc_config['bi_rnn_encoder'], cell_dropout_mode=dropout)
...
class Classifier(tf.keras.Model):
    def __init__(self, class_config, dropout):
        ...
        self.encoder = tx.modules.BidirectionalRNNEncoder(
            hparams = class_config['bi_rnn_encoder'], cell_dropout_mode=dropout)
...
class Embedder(tf.keras.Model):
    def __init__(self, vocab_size, emb_config):
        super(Embedder, self).__init__()
        self.word_embedder = tx.modules.WordEmbedder(vocab_size=vocab_size,
                                        hparams=emb_config["word_embedder"])
        self.posistion_embedder = tx.modules.PositionEmbedder(position_size=emb_config["position_size"],
                                                              hparams=emb_config["position_embedder"])
        self.embedder = lambda tokens, posistions, dropout: \
            self.word_embedder(tokens, mode=dropout) + self.posistion_embedder(posistions, mode=dropout)
...
#         any_labeled = tf.reduce_any(labeled)
        ...
        #
        def get_unsup_data():
            data_batch = unsup_iterator.get_next()
            inp = data_batch['x_text_ids']
            all_data_labels = data_batch['label']
            seq_lengths = data_batch['x_length']
            # 0.5 indicates unsupervised, remove for class
            labeled = tf.logical_not(tf.equal(all_data_labels, -1))
            return inp, all_data_labels, seq_lengths, labeled
         
        def get_nounsup_data():
            
            def get_next_batch():
                data_batch = nounsup_iterator.get_next()
                inp = data_batch['x_text_ids']
                all_data_labels = data_batch['label']
                seq_lengths = data_batch['x_length']
                # 0.5 indicates unsupervised, remove for class
            ...
            def stop_loop(inp, all_data_labels, seq_lengths, labeled):
                no_labeled = False
                return inp, all_data_labels, seq_lengths, labeled
            
            step = 0
            no_labeled = True
            while step < 50 and no_labeled is True: 
                inp, all_data_labels, seq_lengths, labeled = get_next_batch()
                any_labeled = tf.reduce_any(labeled)
                
                inp, all_data_labels, seq_lengths, labeled = tf.cond(
                    any_labeled,
                    lambda: get_next_batch(),
                    lambda: stop_loop(inp, all_data_labels, seq_lengths, labeled)
                    )
                step = step + 1
            return inp, all_data_labels, seq_lengths, labeled
        ...
        inp, all_data_labels, seq_lengths, labeled = tf.cond(
            use_unsup,
            lambda: get_unsup_data(),
            lambda: get_nounsup_data()
            )
...
#                                              dtype=tf.float32)
        ...
        disc_logits_output_layer = tx.core.get_layer(config.disc_hparams["bi_logits_output_layer"])
        disc_cell_output_layer = tx.core.get_layer(config.disc_hparams["bi_cell_output_layer"])
        ...
        clas_logits_output_layer = tx.core.get_layer(config.clas_hparams["bi_logits_output_layer"])
        clas_cell_output_layer = tx.core.get_layer(config.clas_hparams["bi_cell_output_layer"])
...
#                 max_decoding_length=max_length)
            ...
            if sample_strategy == "infer_sample":
                gpt2_context_helper = custom_helpers.GPT2ContextSampleEmbeddingHelper(
                    g_decoder.embeddings(), random_vector, start_tokens, end_token, softmax_temperature)
...
#                 return_cell_output=True)
            ...
            (r_disc_outputs_fw, r_disc_outputs_bw), _, (r_disc_cell_outputs_fw, r_disc_cell_outputs_bw) = discriminator(
                real_inp, sequence_length=real_seq_lengths, return_cell_output=True,
                mode=discriminator_dropout)
            (f_disc_outputs_fw, f_disc_outputs_bw), _, (f_disc_cell_outputs_fw, f_disc_cell_outputs_bw) = discriminator(
                fake_inp, sequence_length=gen_lengths, mode=discriminator_dropout,
                return_cell_output=True)

            # Concatenate outputs of FW and BW
            r_disc_q_logit = disc_logits_output_layer(
                tf.concat([r_disc_outputs_fw, r_disc_outputs_bw], axis=-1)
                )
            f_disc_q_logit = disc_logits_output_layer(
                tf.concat([f_disc_outputs_fw, f_disc_outputs_bw], axis=-1)
                )
            r_disc_cell_outputs = disc_cell_output_layer(
                tf.concat([r_disc_cell_outputs_fw, r_disc_cell_outputs_bw], axis=-1)
                )
            f_disc_cell_outputs = disc_cell_output_layer(
                tf.concat([f_disc_cell_outputs_fw, f_disc_cell_outputs_bw], axis=-1)
                )
...
#             if config.let_discriminator_train_embedder:
            if config.disc_has_own_embedder:
                d_variables = tx.utils.collect_trainable_variables([discriminator, 
                                                                    disc_embedder_model.word_embedder, 
                                                                    disc_embedder_model.posistion_embedder,
                                                                    disc_logits_output_layer,
                                                                    disc_cell_output_layer])
            else:
                d_variables = tx.utils.collect_trainable_variables([discriminator, 
                                                                    disc_logits_output_layer,
                                                                    disc_cell_output_layer])
            ...
            # Initially have to predict value based on no in
...
#                 mode=classifier_dropout, return_cell_output=True)
            (r_clas_outputs_fw, r_clas_outputs_bw), _, (r_clas_cell_outputs_fw, r_clas_cell_outputs_bw) = classifier(
                real_label_inp_emb, sequence_length=label_seq_lengths,
                mode=classifier_dropout, return_cell_output=True)
            (f_clas_outputs_fw, f_clas_outputs_bw), _, (f_clas_cell_outputs_fw, f_clas_cell_outputs_bw) = classifier(
                fake_label_inp_emb, sequence_length=gen_lengths,
                mode=classifier_dropout, return_cell_output=True)

            # Concatenate outputs of FW and BW
            r_clas_q_logit = clas_logits_output_layer(
                tf.concat([r_clas_outputs_fw, r_clas_outputs_bw], axis=-1)
                )
            f_clas_q_logit = clas_logits_output_layer(
                tf.concat([f_clas_outputs_fw, f_clas_outputs_bw], axis=-1)
                )
            r_clas_cell_outputs = clas_cell_output_layer(
                tf.concat([r_clas_cell_outputs_fw, r_clas_cell_outputs_bw], axis=-1)
                )
            f_clas_cell_outputs = clas_cell_output_layer(
                tf.concat([f_clas_cell_outputs_fw, f_clas_cell_outputs_bw], axis=-1)
                )
            ...
            if config.clas_has_own_embedder:
                c_variables = tx.utils.collect_trainable_variables([classifier, 
                                                                    clas_emb_model.word_embedder,
                                                                    clas_emb_model.posistion_embedder,
                                                                    clas_logits_output_layer,
                                                                    clas_cell_output_layer])
            else:
                c_variables = tx.utils.collect_trainable_variables([classifier,
                                                                    clas_logits_output_layer,
                                                                    clas_cell_output_layer])
...
#             pg_variables = tx.utils.collect_trainable_variables([g_decoder, embedder])
            ...
            if config.is_gpt2_trainable == True:
                ...
                pg_variables = tx.utils.collect_trainable_variables([emb_model.word_embedder,
                                                                     emb_model.posistion_embedder])
                pg_train_op = pg_optimizer.minimize(pg_loss,
                                                    global_step=global_step,
                                                    var_list=pg_variables)
...
#                 return
            ...
            for e in range(config.c_pretrain_epochs):
                ...
                total_runtime = total_runtime + clas_rtns["total_runtime"]
                if clas_rtns['real_acc'] > max_acc:
                        max_acc = clas_rtns['real_acc']
                        checkpoint.save(sess, os.path.join(checkpoint_dir, 'ckpt-bestclas-max-acc'))
                if clas_rtns['loss'] < min_loss:
                    min_loss = clas_rtns['loss']
                    checkpoint.save(sess, os.path.join(checkpoint_dir, 'ckpt-bestclas-min-loss'))

            logger.info('\nMin Clas Pretrain val loss: {}, acc: {}'.format(min_loss, max_acc))
            ...
            for e in range(config.adversarial_epochs):
                ...
                logger.info('\nMin Clas Adv val loss: {}, acc: {}'.format(min_loss, max_acc))
            ...
            logger.info("\nTotal GPU Runtime: {}".format(round(total_runtime, 2)))
