...
class Generator(tf.keras.Model):
    ...
    def __init__(self, vocab_size, gen_config, dropout):
        super(Generator, self).__init__()
        self.decoder = tx.modules.BasicRNNDecoder(vocab_size=vocab_size,
                                                  hparams=gen_config['rnn_decoder'],
...
class Discriminator(tf.keras.Model):
    """Discriminator wrapper"""
    def __init__(self, disc_config, vocab_size, cache_dir=None):
        super(Discriminator, self).__init__()
#         disc_config['bert_encoder']['vocab_size'] = vocab_size
        
        if cache_dir is None:
            self.encoder = tx.modules.BERTEncoder(
                hparams = disc_config['bert_encoder'])      
        else:
            self.encoder = tx.modules.BERTEncoder(
                hparams = disc_config['bert_encoder'], cache_dir=cache_dir)  
...
class Classifier(tf.keras.Model):
    """Classifier wrapper"""
    def __init__(self, class_config, vocab_size, cache_dir=None):
        super(Classifier, self).__init__()    
#         class_config['bert_encoder']['vocab_size'] = vocab_size
        
        if cache_dir is None:
            self.encoder = tx.modules.BERTEncoder(
                hparams = class_config['bert_encoder'])      
        else:
            self.encoder = tx.modules.BERTEncoder(
                hparams = class_config['bert_encoder'], cache_dir=cache_dir)  
...
def main(config = None):
    ...
    with g.as_default():
        ...
        # Embedding
        generator_dropout = tf.placeholder(dtype=tf.string)
        emb_model = Embedder(vocab_size, config.emb_hparams)
        embedder = emb_model.embedder
        # Generator
        gen_model = Generator(vocab_size, config.gen_hparams, generator_dropout)
...
#         discriminator = disc_model.encoder
        discriminator_dropout = tf.placeholder(dtype=tf.string)
        disc_model = Discriminator(config.disc_hparams, vocab_size)
        discriminator = disc_model.encoder
...
#             clas_embedder = embedder
        classifier_dropout = tf.placeholder(dtype=tf.string)
        clas_model = Classifier(config.clas_hparams, vocab_size)
        classifier = clas_model.encoder
...
#             real_seq_lengths = tf.clip_by_value(real_seq_lengths, 0, tf.shape(real_inp)[1])
            real_seq_lengths = tf.clip_by_value(real_seq_lengths, 0, tf.shape(real_seq)[1])
#             fake_inp = fake_inp[:, :max_length, :]
#             gen_lengths = tf.clip_by_value(gen_lengths, 0, tf.shape(fake_inp)[1])
            gen_lengths = tf.clip_by_value(gen_lengths, 0, tf.shape(fake_seq)[1])

            if config.add_sentence_progress:
#                 f_progress_vector = tf.ones_like(fake_inp)
                f_progress_vector = tf.ones_like(fake_seq)
...
#                 t_r = tf.shape(real_inp)[1]
                b_f = tf.shape(fake_seq)[0]
                t_f = tf.shape(fake_seq)[1]
                b_r = tf.shape(real_seq)[0]
                t_r = tf.shape(real_seq)[1]
...
#                     [b_r, t_r, 1]), dtype=tf.float32)
                gen_lengths_reshape = tf.cast(tf.reshape(
                    tf.tile(gen_lengths, [t_f]), 
                    [b_f, t_f]), dtype=tf.float32)
                real_seq_lengths_reshape = tf.cast(tf.reshape(
                    tf.tile(real_seq_lengths, [t_r]),
                    [b_r, t_r]), dtype=tf.float32)
...
#                 fake_inp = tf.concat([fake_inp, f_progress_vector], axis = -1)
                real_seq = tf.concat([real_seq, tf.to_int64(r_progress_vector)], axis = -1)
                fake_seq = tf.concat([fake_seq, tf.to_int32(f_progress_vector)], axis = -1)      
                real_seq = tf.cast(real_seq, tf.int32)
...
#             print("check r_disc: ")
            r_disc_outputs, r_disc_pooled_output = discriminator(
                real_seq, sequence_length=real_seq_lengths, mode=discriminator_dropout)
#             print("r_disc is cool!")
#             print("check f_disc: ")
            f_disc_outputs, f_disc_pooled_output = discriminator(
                fake_seq, sequence_length=gen_lengths, mode=discriminator_dropout)
#             print("f_disc is cool!")
            
            if config.use_bert_encoder_pooled_output:            
                r_disc_q_logit = r_disc_pooled_output
                f_disc_q_logit = f_disc_pooled_output
            else:
                disc_dense_layer = tf.layers.Dense(**config.disc_crit_hparams)
                disc_dense = tf.keras.layers.TimeDistributed(disc_dense_layer)
                r_disc_q_logit = disc_dense(r_disc_outputs)
                f_disc_q_logit = disc_dense(f_disc_outputs)
...
#             print("r_disc_crit_inp: {}, r_disc_crit_target: {}, f_disc_crit_inp: {}, f_disc_crit_target: {}, r_disc_crit_baselines: {}, f_disc_crit_baselines: {}"
#                   .format(r_disc_crit_inp, r_disc_crit_target, f_disc_crit_inp, f_disc_crit_target, r_disc_crit_baselines, f_disc_crit_baselines))
#             print("init init_pred: {}".format(init_pred))
#             
...
#             print("reshape init_pred: {}".format(init_pred))
#             print("concat r_disc_crit_baselines: {}".format(r_disc_crit_baselines))
#             print("concat r_disc_crit_baselines: {}".format(f_disc_crit_baselines))
#         
...
#             fake_label_inp_emb = clas_embedder(fake_seq, mode=classifier_dropout)
            ...
            real_label_inp = tf.cast(real_label_inp, tf.int32)
...
#                 mode=classifier_dropout, return_cell_output=True)
            r_clas_outputs, r_clas_pooled_output = classifier(
                real_label_inp, sequence_length=label_seq_lengths, mode=classifier_dropout)
            f_clas_outputs, f_clas_pooled_output = classifier(
                fake_seq, sequence_length=gen_lengths, mode=classifier_dropout)
            
            if config.use_bert_encoder_pooled_output:
                r_clas_q_logit = r_clas_pooled_output
                f_clas_q_logit = f_clas_pooled_output
            else:
                clas_dense_layer = tf.layers.Dense(**config.clas_crit_hparams)
                clas_dense = tf.keras.layers.TimeDistributed(clas_dense_layer)
            
                r_clas_q_logit = disc_dense(r_clas_outputs)
                r_clas_q_logit_notime = clas_dense_layer(r_clas_outputs)
                
                print("r_clas_q_logit: {}".format(r_clas_q_logit))
                print("r_clas_q_logit_notime: {}".format(r_clas_q_logit_notime))
                
                f_clas_q_logit = disc_dense(f_clas_outputs)
...
#             print("after squeeze advantages: {}".format(advantages))
...
#                 print("linear_decay_pg_weights advantages: {}".format(advantages))
            ...
            if config.norm_advantages:
                advantages = tx.losses.discount_reward(advantages, 
                                                       ...
                                                       normalize=True,
                                                       tensor_rank=2)
#                 print("norm_advantages advantages: {}".format(advantages))
            
            advantages = tf.clip_by_value(advantages, -config.adv_max_clip, config.adv_max_clip)

#             print("before pg_loss advantages: {}".format(advantages))
#             print("gen_lengths: {}".format(gen_lengths))
...
#                                   'V_c', 'Q', 'A', 'pgloss']
                        header = ['tkn', 'logit', 'log_prob', 'Q_d', 'Q_c', 'Q', 'A', 'pgloss']
                        values = [list(vocab.map_ids_to_tokens_py(rtns['sentence'])),
            ...
            for e in range(config.adversarial_epochs):
                ...
                if config.mle_loss_in_adv:
                    for i in range(config.gen_mle_adv_epoch): 
                        ...
                        logger.info('\nGen Adv-MLE Val Epoch{}'.format(cur_epoch))
                        gen_rtns = gen_run_epoch(sess, 'val', sum_writer)
                        logger.info('\nGen Val loss:{}'.format(gen_rtns['loss']))
                ...
                while config.discriminator_loss_lambda > 0 and disc_e < config.disc_adv:
                    ...
                    logger.info('\nDisc Adv-Val Epoch: {}+{}'.format(cur_epoch, disc_e))
