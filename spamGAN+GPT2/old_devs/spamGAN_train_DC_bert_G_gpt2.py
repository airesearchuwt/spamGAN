...
class Generator(tf.keras.Model):
    """Generator wrapper for checkpointing"""
    def __init__(self, vocab_size, gen_config, cache_dir=None):
        super(Generator, self).__init__()
        gen_config['gpt2_decoder']['vocab_size'] = vocab_size
        
        if cache_dir is None:
            self.decoder = tx.modules.GPT2Decoder(
                hparams=gen_config['gpt2_decoder'])      
        else:
            self.decoder = tx.modules.GPT2Decoder(
                hparams=gen_config['gpt2_decoder'], cache_dir=cache_dir)  
...
class Discriminator(tf.keras.Model):
    """Discriminator wrapper"""
    def __init__(self, vocab_size, disc_config, cache_dir=None):
...
class Classifier(tf.keras.Model):
    """Classifier wrapper"""
    def __init__(self, vocab_size, class_config, cache_dir=None):
...
#                                              dtype=tf.float32)
        ...
        gen_model = Generator(vocab_size, config.gen_hparams)
        g_decoder = gen_model.decoder
...
#         discriminator = disc_model.encoder
        discriminator_dropout = tf.placeholder(dtype=tf.string)
        disc_model = Discriminator(vocab_size, config.disc_hparams)
        discriminator = disc_model.encoder
...
#             clas_embedder = embedder
        classifier_dropout = tf.placeholder(dtype=tf.string)
        clas_model = Classifier(vocab_size, config.clas_hparams)
        classifier = clas_model.encoder
...
#                 mode=classifier_dropout, return_cell_output=True)
            ...
            if config.use_bert_encoder_pooled_output:
                ...
                r_clas_q_logit = clas_dense(r_clas_outputs)
                f_clas_q_logit = clas_dense(f_clas_outputs)
