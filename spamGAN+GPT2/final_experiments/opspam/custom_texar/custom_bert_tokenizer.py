...
from custom_tokenizer_base import TokenizerBase
...
class BERTTokenizer(PretrainedBERTMixin, TokenizerBase):
    ...
    def encode_tokens(self,
        ...
        cls_token_id = tf.convert_to_tensor(
            self._map_token_to_id(self.cls_token)
            )
        sep_token_id = tf.convert_to_tensor(
            self._map_token_to_id(self.sep_token)
            )

        token_ids_a = self.map_token_to_id(tokens_a)

        token_ids_b = None
        if tokens_b is not None:
            token_ids_b = self.map_token_to_id(tokens_b)
