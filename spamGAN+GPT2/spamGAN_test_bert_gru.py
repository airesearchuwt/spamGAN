...
class BERTEmbedder(tf.keras.Model):
    def __init__(self,vocab_size, bert_emb_config):
        super(BERTEmbedder, self).__init__()
#         bert_emb_config["vocab_size"] = vocab_size
        self.embedder = tx.modules.BERTEncoder(hparams=bert_emb_config)
...
#         config = importlib.import_module('spamGAN_config_smallunsup')
    ...
    with g.as_default():
        ...
        bert_emb_model = BERTEmbedder(vocab_size, config.bert_emb_hparams)
...
#             disc_embedder = disc_embedder_model.embedder
            r_disc_embedder_model = BERTEmbedder(vocab_size, config.bert_emb_hparams)
            r_disc_embedder = r_disc_embedder_model.embedder
            f_disc_embedder_model = BERTEmbedder(vocab_size, config.bert_emb_hparams)
            f_disc_embedder = f_disc_embedder_model.embedder
        else:
...
#             clas_embedder = clas_emb_model.embedder
            r_clas_emb_model = BERTEmbedder(vocab_size, config.bert_emb_hparams)
            r_clas_embedder = r_clas_emb_model.embedder
            f_clas_emb_model = BERTEmbedder(vocab_size, config.bert_emb_hparams)
            f_clas_embedder = f_clas_emb_model.embedder
        else:
...
#             fake_inp = disc_embedder(fake_seq, mode=discriminator_dropout)
            real_inp, _ = r_disc_embedder(real_seq, mode=discriminator_dropout)
            fake_inp, _ = f_disc_embedder(fake_seq, mode=discriminator_dropout)
...
#             fake_label_inp_emb = clas_embedder(fake_seq, mode=classifier_dropout)
            real_label_inp_emb, _ = r_clas_embedder(real_label_inp, mode=classifier_dropout)
            fake_label_inp_emb, _ = f_clas_embedder(fake_seq, mode=classifier_dropout)
