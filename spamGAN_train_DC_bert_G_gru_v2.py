...
class Discriminator(tf.keras.Model):
    """Discriminator wrapper"""
    def __init__(self, disc_config, cache_dir=None):
        ...
        drop_kwargs = {"rate": disc_config["output_layers"]["dropout"]}
        dropout_layer_hparams = {"type": "Dropout", "kwargs": drop_kwargs}
        self._dropout_layer = get_layer(hparams=dropout_layer_hparams)
        
        # Creates an additional classification layer
        logit_kwargs = disc_config["output_layers"]["logit_layer_kwargs"]
        if logit_kwargs is None:
            logit_kwargs = {}
        else:
            logit_kwargs = logit_kwargs.todict()
            
        logit_kwargs.update({"units": disc_config["output_layers"]["num_classes"]})
        if 'name' not in logit_kwargs:
            logit_kwargs['name'] = "logit_layer"

        dense_layer_hparams = {"type": "Dense", "kwargs": logit_kwargs}
        self._logit_layer = get_layer(hparams=dense_layer_hparams)
...
class Classifier(tf.keras.Model):
    """Classifier wrapper"""
    def __init__(self, class_config, cache_dir=None):
        ...
        # Creates an dropout layer
        drop_kwargs = {"rate": class_config["output_layers"]["dropout"]}
        dropout_layer_hparams = {"type": "Dropout", "kwargs": drop_kwargs}
        self._dropout_layer = get_layer(hparams=dropout_layer_hparams)
        
        # Creates an additional classification layer
        logit_kwargs = class_config["output_layers"]["logit_layer_kwargs"]
        if logit_kwargs is None:
            logit_kwargs = {}
        else:
            logit_kwargs = logit_kwargs.todict()
            
        logit_kwargs.update({"units": class_config["output_layers"]["num_classes"]})
        if 'name' not in logit_kwargs:
            logit_kwargs['name'] = "logit_layer"
...
def main(config = None):
    ...
    if config is None:
        ...
        config = importlib.import_module('spamGAN_config_minimal_gpu')
    else:
...
#         discriminator = disc_model.encoder
        ...
        discriminator = disc_model.encoder
        disc_dropout_layer = disc_model._dropout_layer
        disc_logit_layer = disc_model._logit_layer
...
#             clas_embedder = embedder
        ...
        classifier = clas_model.encoder
        clas_dropout_layer = clas_model._dropout_layer
        clas_logit_layer = clas_model._logit_layer
...
#                 return_cell_output=True)
            r_disc_outputs, _ = discriminator(
                real_seq, sequence_length=real_seq_lengths, mode=discriminator_dropout)
            f_disc_outputs, _ = discriminator(
                fake_seq, sequence_length=gen_lengths, mode=discriminator_dropout)
            
            r_disc_q_logit = disc_dropout_layer(r_disc_outputs, training=tf.placeholder_with_default(False, shape=()))
            r_disc_q_logit = disc_logit_layer(r_disc_q_logit)
            f_disc_q_logit = disc_dropout_layer(f_disc_outputs, training=tf.placeholder_with_default(False, shape=()))
            f_disc_q_logit = disc_logit_layer(f_disc_q_logit)
...
#                 d_variables = tx.utils.collect_trainable_variables([discriminator, disc_embedder])
                d_variables = [disc_dropout_layer.trainable_variables, disc_logit_layer.trainable_variables]
            else:
...
#                 mode=classifier_dropout, return_cell_output=True)
            r_clas_outputs, _ = classifier(
                real_label_inp, sequence_length=label_seq_lengths, mode=classifier_dropout)
            f_clas_outputs, _ = classifier(
                fake_seq, sequence_length=gen_lengths, mode=classifier_dropout)
            
            r_clas_q_logit = clas_dropout_layer(r_clas_outputs, training=tf.placeholder_with_default(False, shape=()))
            r_clas_q_logit = clas_logit_layer(r_clas_q_logit)
            f_clas_q_logit = clas_dropout_layer(f_clas_outputs, training=tf.placeholder_with_default(False, shape=()))
            f_clas_q_logit = clas_logit_layer(f_clas_q_logit)
...
#                 c_variables = tx.utils.collect_trainable_variables([classifier, clas_embedder])
                c_variables = [clas_dropout_layer.trainable_variables, clas_logit_layer.trainable_variables]
            else:
