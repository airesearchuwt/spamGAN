...
class Discriminator(tf.keras.Model):
    ...
    def __init__(self, disc_config):
        ...
        self.encoder = tx.modules.XLNetEncoder(
            hparams = disc_config['xlnet_encoder'])
...
class Classifier(tf.keras.Model):
    def __init__(self, class_config):
        ...
        self.encoder = tx.modules.XLNetEncoder(
            hparams = class_config['xlnet_encoder'])
...
#         config = importlib.import_module('spamGAN_config_minimal')
        config = importlib.import_module('spamGAN_config_smallunsup')
    else:
...
#             clas_embedder = embedder
        ...
        # Output layers
        disc_output_layer = layers.get_layer(config.disc_hparams["output_layer"])
        clas_output_layer = layers.get_layer(config.clas_hparams["output_layer"])
...
#                 return_cell_output=True)
            ...
            r_disc_cell_outputs, _ = discriminator(
                real_seq,
                mode=discriminator_dropout
                )
            r_disc_q_logit = disc_output_layer(r_disc_cell_outputs)
            
            f_disc_cell_outputs, _ = discriminator(
                fake_seq, 
                mode=discriminator_dropout
                )
            f_disc_q_logit = disc_output_layer(f_disc_cell_outputs)
...
#                 d_variables = tx.utils.collect_trainable_variables([discriminator, disc_embedder])
                ...
                d_variables = tx.utils.collect_trainable_variables([disc_output_layer])
...
#                 mode=classifier_dropout, return_cell_output=True)
            ...
            r_clas_cell_outputs, _ = classifier(
                real_label_inp,
                mode=classifier_dropout
                )
            r_clas_q_logit = clas_output_layer(r_clas_cell_outputs)
            
            f_clas_cell_outputs, _ = classifier(
                fake_seq, 
                mode=classifier_dropout
                )
            f_clas_q_logit = clas_output_layer(f_clas_cell_outputs)
...
#                 c_variables = tx.utils.collect_trainable_variables([classifier, clas_embedder])
                ...
                c_variables = tx.utils.collect_trainable_variables([clas_output_layer])
